...
def topic_talker(point_x, point_y, angle):
    ...
    z = angle 
    msg = Point(x,y,z)
    pub_point_xy.publish(msg)

    # rate = rospy.Rate(10)
    # rate.sleep()
#以下發布正反面訊息
def topic_front_reverse(label):
    global pub_front_reverse

    label_info = label
    pub_front_reverse.publish(label_info)
...
#標定治具點位的傳輸
def place_array_xy(Arr_xy):
    global pub_Place_xy

    msg = Float32MultiArray() #多維陣列
    msg.layout.dim.append(MultiArrayDimension('x', 20, 4))#多維陣列宣告維度
    msg.data = Arr_xy
    pub_Place_xy.publish(msg)
...
#補料訊息傳輸
def refill(isplaced_list):
    global pub_refill
    
    msg=Float32MultiArray()#
    msg.layout.dim.append(MultiArrayDimension('NG',10,1))
    msg.data=isplaced_list
    pub_refill.publish(msg)


def main1():

    # T_base_e = transform_mtx(3.74, -538.35, 250.00, -179.19, 1.07, -0.02) #法蘭面座標系
    ...
    topic_front_reverse(label_sort[0])#信心最高的參數發送 發布正反面訊息 
    topic_Ry(feature_angle[0])  #信心最高的參數發送 膠體開口上下之手臂傾角姿態 +-45度
    topic_talker(Move_ptsx[0][0],Move_ptsy[0][0],rot_angle[0]) #信心最高的參數發送 發布膠體位置訊息
...
def main2():
        ...
        Move_ptsx,Move_ptsy = trans_obj(places, T_base_camera, height=189.8915)#要移動到的世界座標點位 ；height為s 跟高度有關 height=176.9366
        M_Pts = np.append(Move_ptsx,Move_ptsy)

        place_array_xy(M_Pts)
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--img", default=str(ASSETS / "pick.jpg"), help="Path to input image.")
