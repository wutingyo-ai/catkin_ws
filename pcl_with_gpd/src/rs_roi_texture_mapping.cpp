...
try
{
    ...
    // ros::Publisher samples_pub = nh.advertise<gpd_ros::SamplesMsg>("/sample", 1);
    // ros::Publisher cloud_samples_pub = nh.advertise<gpd_ros::CloudSamples>("/cloud_stitched", 1);
    ...
    while (0 <= frame_count && frame_count < 20)
    {
        ...
        // auto intrinsics = color_frame.get_profile().as<rs2::video_stream_profile>().get_intrinsics();
            ...
            for (int x = roi.x; x < roi.x + roi.width; ++x)
            {
                ...
                if (depth_value > 0 && depth_value < 3) // 確保深度有效 0.69 m 深度
                {
                    ...
                    if (depth_value < min_depth)
                    {
                        ...
                        // 更新最近的點
                        closest_point.x = point[0];
                        closest_point.y = point[1];
                        closest_point.z = point[2];
       ...
       /*  // 創建 CloudSources 消息
        ...
        camera_index.data = 0;                               // 設定相機索引
        cloud_sources.camera_source.push_back(camera_index); // 添加相機索引

        // 添加相機位置
        geometry_msgs::Point view_point;
        view_point.x = 0; // 根據需要設置相機位置
        view_point.y = 0;
        view_point.z = 0;
        cloud_sources.view_points.push_back(view_point); // 添加相機位置
        ...
        // 計算 ROI 區域的中心點
        int center_x = x_roi + width_roi / 2;
        int center_y = y_roi + height_roi / 2;
        // 獲取該中心點的深度值
        float depth_value = depth_frame.get_distance(center_x, center_y); // 獲取該像素的深度值
                                                                          // 確保深度值有效
        if (depth_value > 0)
        {
            // 轉換 2D 像素座標為 3D 座標
            float pixel[2] = {static_cast<float>(center_x), static_cast<float>(center_y)};
            float point[3];
            rs2_deproject_pixel_to_point(point, &intrinsics, pixel, depth_value);
            // 添加樣本點 (這裡假設您有一些樣本點)
            // geometry_msgs::Point sample_point;
            // sample_point.x = point[0]; // 3D 點的 x 坐標
            // sample_point.y = point[1]; // 3D 點的 y 坐標
            // sample_point.z = point[2]; // 3D 點的 z 坐標
            // cloud_samples.samples.push_back(sample_point); // 添加樣本點
        }
        cloud_samples.samples.push_back(closest_point); // 添加樣本點
 */
        /*  // 計算 ROI 區域的中心點
         int center_x = x_roi + width_roi / 2;
         int center_y = y_roi + height_roi / 2;
         ...
         gpd_ros::SamplesMsg sample_other_topic;
         ...
         if (depth_value > 0)
         {
             ...
             sample_point.y = point[1];                          // 3D 點的 y 坐標
             sample_point.z = point[2];                          // 3D 點的 z 坐標
             sample_other_topic.samples.push_back(sample_point); // 添加樣本點
         }

         sample_other_topic.header.frame_id = "base_link";
         sample_other_topic.header.stamp = ros::Time::now(); // 設定時間戳

         // samples_pub.publish(sample_other_topic);
         cloud_samples_pub.publish(output); */
        

        cloud_samples_pub.publish(output);
        // cloud_samples_pub.publish(cloud_samples);
        std::cin.get();
        // break;
